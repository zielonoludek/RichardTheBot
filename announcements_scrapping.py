...
try:
    requests.get(base_url, headers=payload)
    ...
    with requests.Session() as s:
        for p in range(page, 0, -1):
            ...
            for link in soup.find('ul', class_='newsgrid-list').find_all('a'):
                archive['announcements'].append({"page": p, "id": id, "title": link["title"], "href": link["href"], "img": link.find('img')['src'], "content": link.find('div', class_='text').text})
                id+=1
